...
func _on_all_players_spawned():
	pass # All players have been spawned and positioned
...
func position_players_at_start(player_group_name = "players"):
	...
	for i in range(players.size()):
		...
		# Set position exactly on ground level to prevent jumping
		...
		# Explicitly force position on ALL clients for this player
		if multiplayer.has_multiplayer_peer() and player.has_method("rpc"):
			if "player_id" in player:
				player.rpc("sync_position", transform.origin, player.global_transform.basis.get_rotation_quaternion())
